...
UA_Browser="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36";
UA_Dalvik="Dalvik/2.1.0 (Linux; U; Android 11; LE2110 Build/RKQ1.201105.002)";
...
local_ipv4=$(curl $useNIC -4 -s --interface 172.16.0.2 --max-time 10 api64.ipify.org)
local_ipv6=$(curl $useNIC -6 -s --max-time 20 api64.ipify.org)
local_isp4=$(curl $useNIC -s -4 --interface 172.16.0.2 --max-time 10 https://api.ip.sb/geoip/${local_ipv4} | cut -f1 -d"," | cut -f4 -d '"')
...
function MediaUnlockTest_Netflix() {
    echo -n -e " Netflix:\t\t\t\t->\c";
    local result1=$(curl $useNIC -${1} --user-agent "${UA_Browser}" -fsL --write-out %{http_code} --interface 172.16.0.2 --output /dev/null --max-time 10 "https://www.netflix.com/title/81215567" 2>&1)
    ...
    if [[ "$result1" == "404" ]];then
		...
		local region=`tr [:lower:] [:upper:] <<< $(curl $useNIC -${1} --user-agent "${UA_Browser}" -fs --max-time 10 --interface 172.16.0.2 --write-out %{redirect_url} --output /dev/null "https://www.netflix.com/title/80018499" | cut -d '/' -f4 | cut -d '-' -f1)` ;
		if [[ ! -n "$region" ]];then
			region="US";
		fi
...
function CheckV4() {
	if [[ "$language" == "e" ]];then
		if [[ "$NetworkType" == "6" ]];then
			...
			echo -e " ${Font_SkyBlue}** Your Network IP: ${local_ipv4}${Font_Suffix} "
